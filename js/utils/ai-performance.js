 ...
 */
async function batchAIOperations(operations, session, batchSize = 5) {
 ...
 */
function createOptimizedPrompt(operation, content) {
 ...
 * @param {Object} session - AI session object
 */
async function prewarmAISession(session) {
  try {
...
function updatePerformanceMetrics(duration, operationCount) {
  ...
  aiOperationMetrics.averageLatency =
    currentAvg === 0 ? newLatency : currentAvg * 0.7 + newLatency * 0.3;
}
 ...
 */
function resetAIPerformanceMetrics() {
 ...
 * @returns {string} Optimized prompt
 */
function optimizePromptForModel(prompt) {
...
window.batchAIOperations = batchAIOperations;
window.createOptimizedPrompt = createOptimizedPrompt;
window.prewarmAISession = prewarmAISession;
window.getAIPerformanceMetrics = getAIPerformanceMetrics;
window.resetAIPerformanceMetrics = resetAIPerformanceMetrics;
window.optimizePromptForModel = optimizePromptForModel;
