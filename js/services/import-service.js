...
// Import functionality for journal entries from various formats

import * as store from '../core/store.js';
import { generateId } from '../utils/utils.js';

/**
 * Validates and imports journal data from JSON
...
/**
 * Imports journal data from plain text format
    ...
    try {
        ...
        for (const entry of entries) {
            try {
                ...
                errors.push(`Failed to import entry "${entry.title}": ${error.message}`);
            }
        }
...
function validateAndNormalizeEntry(entry, index) {
    ...
    const normalized = {
        ...
        oneLiner: entry.oneLiner || entry.summary || 'Imported journal entry',
        createdAt: validateDate(entry.createdAt) || new Date().toISOString(),
        updatedAt: validateDate(entry.updatedAt) || validateDate(entry.createdAt) || new Date().toISOString()
    };
    ...
    if (entry.topics && Array.isArray(entry.topics)) {
        normalized.topics = entry.topics.filter(topic => typeof topic === 'string');
    }
...
export function validateImportData(data) {
    ...
    try {
        ...
        if (typeof data === 'object' && data.entries) {
            ...
            validation.entriesCount = Array.isArray(data.entries) ? data.entries.length : 0;
...
export async function createPreImportBackup(currentNotes) {
    try {
        const { exportToJSON, downloadFile, generateExportFilename } = await import('./export-service.js');
