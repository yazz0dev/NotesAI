...
function updateHabitStreak(goal, today) {
    ...
    for (let i = 0; i < recentCheckIns.length; i++) {
        ...
        const daysDiff = Math.floor((todayDate - checkInDate) / (1000 * 60 * 60 * 24));
...
async function storeGoal(goal) {
    try {
        ...
        return new Promise((resolve, reject) => {
            request.onerror = () => reject(new Error('Failed to open goals database'));
        ...
        return allGoals.filter(goal => {
            ...
            if (goal.type === GOAL_TYPES.HABIT) {
                const lastCompleted = goal.lastCompleted ? 
                    new Date(goal.lastCompleted).toISOString().split('T')[0] : null;
                ...
                if (goal.frequency === HABIT_FREQUENCIES.WEEKLY && lastCompleted) {
                    const daysSince = Math.floor((new Date(today) - new Date(lastCompleted)) / (1000 * 60 * 60 * 24));
                    return daysSince >= 7;
                }
...
export async function generateGoalInsights(goals, recentEntries = []) {
    try {
        ...
        const goalsData = goals.map(goal => ({
            ...
            type: goal.type,
            status: goal.status,
            progress: goal.targetValue ? `${goal.currentValue}/${goal.targetValue}` : goal.currentValue,
        ...
        return {
            ...
            completedGoals: goals.filter(g => g.status === GOAL_STATUS.COMPLETED).length,
...
function getFallbackInsights(goals) {
    ...
    return {
        ...
        strengths: [
            completedGoals.length > 0 ? 'Great job completing goals!' : 'You\'re building momentum',
            totalStreak > 0 ? `${totalStreak} total habit streak days` : 'Starting your habit journey'
        ],
        opportunities: [
            'Consider breaking large goals into smaller milestones',
            'Reflect on your progress in your journal entries'
        ],
        streakHighlight: longestStreak > 0 ? 
