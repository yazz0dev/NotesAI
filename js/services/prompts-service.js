...
const PROMPT_CATEGORIES = {
    ...
    reflection: {
        ...
        themes: ['personal growth', 'lessons learned', 'self-awareness', 'introspection']
    },
    goals: {
 ...
 */
export async function generateDailyPrompts(sentiment = 'neutral', recentTopics = [], timeOfDay = 'morning') {
    try {
        // Check if Chrome AI is available
        if (!chrome?.ai?.prompt) {
            console.warn("Chrome AI API not available for prompt generation, using fallback");
        ...
        const recentTopicsStr = recentTopics.length > 0 ? recentTopics.join(', ') : 'general life';
        ...
        const prompts = JSON.parse(result.text.trim());
        
        // Validate and return
        return Array.isArray(prompts) ? prompts.slice(0, 3) : getFallbackPrompts(sentiment, timeOfDay);
...
function selectCategoryBySentiment(sentiment, timeOfDay) {
    if (sentiment === 'negative') {
        return timeOfDay === 'evening' ? PROMPT_CATEGORIES.reflection : PROMPT_CATEGORIES.wellbeing;
    }
    
    if (sentiment === 'positive') {
        return timeOfDay === 'morning' ? PROMPT_CATEGORIES.goals : PROMPT_CATEGORIES.gratitude;
    }
    
    // Neutral sentiment - vary by time of day
    switch (timeOfDay) {
        case 'morning': return PROMPT_CATEGORIES.goals;
        case 'afternoon': return PROMPT_CATEGORIES.creativity;
        case 'evening': return PROMPT_CATEGORIES.reflection;
        default: return PROMPT_CATEGORIES.reflection;
    }
}
...
export async function generateWeeklyReflection(weekEntries) {
    try {
        ...
        const weekContent = weekEntries.map(entry => {
        ...
        const prompts = JSON.parse(result.text.trim());
        return Array.isArray(prompts) ? prompts.slice(0, 3) : getWeeklyFallbackPrompts();
