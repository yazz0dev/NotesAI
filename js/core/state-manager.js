...
class StateManager {
  constructor() {
    ...
    this.persistenceKey = "notes_tasks_state";
  ...
  getInitialState() {
    return {
      ...
      // UI state
      ui: {
        view: "notes", // notes, editor, settings
        theme: "dark",
        sidebarOpen: false,
        modalsOpen: [],
        searchQuery: "",
  ...
  subscribe(paths, callback, options = {}) {
    const { immediate = false, debounce = 0, filter = null } = options;
  ...
  createActions() {
    return {
      ...
      closeModal: (modalId) =>
        this.setState((state) => ({
          ui: {
            modalsOpen: state.ui.modalsOpen.filter((id) => id !== modalId),
          },
        })),
      ...
      deleteNote: (noteId) =>
        this.setState((state) => ({
          data: {
            notes: state.data.notes.filter((note) => note.id !== noteId),
          },
        })),
      setSearchResults: (results) =>
        this.setState({ data: { searchResults: results } }),
      setRecentTopics: (topics) =>
        this.setState({ data: { recentTopics: topics } }),
      setSentiment: (sentiment) =>
        this.setState({ data: { currentSentiment: sentiment } }),

      // Audio actions
      setRecording: (isRecording, data = null) =>
        this.setState({
      ...
      setAmbientMode: (isAmbientMode) =>
        this.setState({ voice: { isAmbientMode } }),
      setLastCommand: (command) =>
        this.setState({ voice: { lastCommand: command } }),
    ...
    for (const subscription of this.subscribers.values()) {
      ...
      try {
        ...
        if (changedPaths.length > 0) {
          ...
          if (
            subscription.filter &&
            !subscription.filter(newValues, oldValues, newState)
          ) {
            continue;
          }
