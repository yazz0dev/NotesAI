...
class VirtualScrollManager {
  ...
  async initialize(items = []) {
    return window.withErrorHandling(
      async () => {
        if (!this.container) {
          throw new Error(
            "Container element is required for virtual scrolling"
          );
        }
        ...
        console.log(
          `Virtual scrolling ${this.isEnabled ? "enabled" : "disabled"} for ${
            this.items.length
          } items`
        );
      },
      {
        ...
        context: {
          operation: "initializeVirtualScrolling",
  ...
  async renderVisibleItems() {
    ...
    return window.withErrorHandling(
      ...
      {
        type: window.ERROR_TYPES.UNKNOWN,
        severity: window.ERROR_SEVERITY.LOW,
        context: {
 ...
 */
function createJournalVirtualScroll(container, renderCard) {
 ...
 * @returns {VirtualScrollManager} Virtual scroll manager instance
 */
function createGoalsVirtualScroll(container, renderGoal) {
  return new VirtualScrollManager({
    container,
...
window.createJournalVirtualScroll = createJournalVirtualScroll;
window.createGoalsVirtualScroll = createGoalsVirtualScroll;
