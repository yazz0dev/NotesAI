  ...
  try {
    ...
    audioContainer.innerHTML = `
            <div class="audio-header">
                <span class="audio-icon"><span class="iconify" data-icon="material-symbols:music-note"></span></span>
                <span class="audio-label">Audio Recording</span>
                <span class="audio-duration">${formatDuration(
...
export async function openBook(note) {
  ...
  viewerDateEl.textContent = new Date(note.createdAt).toLocaleDateString(
    "en-US",
    {
      year: "numeric",
      month: "long",
...
export function closeBook() {
  ...
  setTimeout(() => {
    ...
    const audioContainer = bookViewerEl.querySelector(
...
export function insertImageIntoViewer(imageDataUrl, caption) {
  ...
  if (selection.rangeCount > 0) {
    ...
    if (
      viewerTextRightEl.contains(container) ||
      viewerTextRightEl === container
    ) {
