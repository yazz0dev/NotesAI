...
import { getAllGoals, createGoal, recordProgress, getGoalsDueToday, generateGoalInsights, GOAL_TYPES, GOAL_STATUS } from '../services/goals-service.js';

let currentEditingGoal = null;

/**
 * Shows the goals modal with dashboard
 */
export async function showGoalsModal(showForm = false) {
...
function populateGoalForm(goal) {
    ...
    document.getElementById('goal-frequency-select').value = goal.frequency || 'daily';
    document.getElementById('goal-target-input').value = goal.targetValue || 1;
    
    if (goal.targetDate) {
        document.getElementById('goal-target-date-input').value = goal.targetDate.split('T')[0];
    }
...
export async function handleGoalFormSubmit(event, notes = []) {
    ...
    try {
        ...
        const formData = {
            ...
            targetValue: parseInt(document.getElementById('goal-target-input').value) || 1,
            targetDate: document.getElementById('goal-target-date-input').value || null
        };
...
function createGoalItemHTML(goal) {
    ...
    return `
        <div class="goal-item ${isCompleted ? 'completed' : ''}" data-goal-id="${goal.id}">
            ...
            <div class="goal-actions">
                ${!isCompleted ? `
                    <button class="goal-action-btn primary goal-progress-btn">
                        ${goal.type === GOAL_TYPES.HABIT ? '✅ Mark Done' : '➕ Add Progress'}
                    </button>
                ` : ''}
            ...
            goalsToday.innerHTML = `
                ...
                ${goalsDue.map(goal => `
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <span style="font-size: 0.9rem; color: var(--text-primary);">${goal.title}</span>
                        <button data-goal-id="${goal.id}" class="quick-progress-btn"
                            ...
                            ${goal.type === GOAL_TYPES.HABIT ? 'Done' : 'Progress'}
                        </button>
                    </div>
                `).join('')}
            `;
