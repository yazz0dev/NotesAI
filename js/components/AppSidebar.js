export default {
    ...
    template: `
      ...
      <nav class="sidebar border-end shadow-sm d-flex flex-column">
        ...
        <!-- Sidebar Footer with Tag Creation -->
        <div class="sidebar-footer border-top" :class="{ 'p-2': isSidebarCollapsed, 'p-3': !isSidebarCollapsed }">
            <div v-if="!isSidebarCollapsed" class="input-group input-group-sm mb-2">
                <input
                    type="text"
                    class="form-control"
                    placeholder="Create new tag..."
                    v-model="localTagName"
                    @keydown.enter="$emit('create-tag', localTagName)"
                    style="border-radius: 6px 0 0 6px;"
                >
                <button class="btn btn-outline-secondary"
                        :disabled="!localTagName.trim()"
                        @click="$emit('create-tag', localTagName)"
